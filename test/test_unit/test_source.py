...
def test_source():
    ...
    Sig11 = (sigs[0,0] + chi[0,0]*nu[0]*sigf[0]/keff) # transfer from 1 to 1
    Sig12 = (sigs[0,1] + chi[0,1]*nu[1]*sigf[1]/keff) # transfer from 2 to 1 (down scatter)
    Sig21 = (sigs[1,0] + chi[1,0]*nu[0]*sigf[0]/keff) # transfer from 1 to 2 (up scatter)
    Sig22 = (sigs[1,1] + chi[1,1]*nu[1]*sigf[1]/keff) # transfer from 2 to 2
    ...
    fission = np.array(((chi[0,0]*nu[0]*sigf[0]/keff*phi0_1 + chi[0,1]*nu[1]*sigf[1]/keff*phi0_2),
                        (chi[1,0]*nu[0]*sigf[0]/keff*phi0_1 + chi[1,1]*nu[1]*sigf[1]/keff*phi0_2)))
    ...
    assert(np.allclose((scatter-scatter_src), 0.0))
    assert(np.allclose((fission-fission_src), 0.0))
    assert(np.allclose((phi1-phi1_src), 0.0))
