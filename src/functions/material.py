...
class Material:
    def __init__(self, material_code, geometry, mesh):
        ...
        if (material_code == "garcia_data"):
            ...
            self.sigt, self.sigs, self.siga, self.G \
                = garcia_data(self.mesh, self.Nx)
            self.media = 1

        elif (material_code == "reeds_data"):
            from src.materials.reeds_data import reeds_data
            self.sigt, self.sigs, self.siga, self.source, self.G \
                = reeds_data(self.Nx, LB=self.LB, RB=self.RB)
            self.media = 9
